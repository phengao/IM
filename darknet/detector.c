...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    fprintf(stdout, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
    srand(time(0));
...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    for (i = 0; i < m; ++i) {
        ...
        //fprintf(stdout, " %s - %s - ", paths[i], labelpath);
        fprintf(stdout, "%5d %5d %5d\tRPs/Img: %.2f\tIOU: %.2f%%\tRecall:%.2f%%\n", i, correct, total, (float)proposals / (i + 1), avg_iou * 100 / total, 100.*correct / total);
        free(id);
        free_image(orig);
        free_image(sized);
    }
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *listname, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *wfile)
{
	char *outfile = { 0 };
	char filename[256] = { 0 };
	//char szTest[256] = { 0 };
	...
	printf("detector :lsitname:<%s>  wfile:<%s>", listname, wfile);
    ...
    int j;
    float nms = .45;    // 0.4F
	// GET FILENAMES TO DETECT


	FILE *fp = fopen(listname, "r");
	FILE *fpw;
	if (wfile) {// use the wfile name by parameter
		fpw = fopen(wfile, "w");
	}
	else {
		fpw = fopen("Detctedlist.txt", "w");
	}

	if (NULL == fp)
	{
		printf("failed to open listname.txt\n");
		return 1;
	}
	if (NULL == fpw)
	{
		printf("failed to open Detcted list <%s>\n",wfile);
		return 1;
	}
	...
	while (!feof(fp))
	{
		...
		fgets(filename, sizeof(filename) - 1, fp); // °üº¬ÁË»»ÐÐ·û  
		//printf("<%s>", filename);
		if (strlen(filename) > 5) { //x.jpg
			if (filename[strlen(filename) - 2] == '\x0d')
				filename[strlen(filename) - 2] = 0;
			else if (filename[strlen(filename) - 1] == '\x0a')
				filename[strlen(filename) - 1] = 0;
			else
				filename[sizeof(filename) - 1] = 0;// the name is too long,must keep a 0 at the end of char array
		}
		else
			continue;// the name is too short
		//printf("<%s> -2:<%x><%x><%x>\n", szTest, szTest[strlen(szTest) - 2], szTest[strlen(szTest) - 1], szTest[strlen(szTest) ]);
		//continue;
    
        if (filename) {
            strncpy(input, filename, 256);// 256 is the orignal writer defined
            if (strlen(input) > 0)
        ...
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
       // draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);// in this function will draw the box
        ...
        save_image(im, "predictions-detec");
        if (!dont_show) {
			;//show_image(im, "predictions-detec");
        }
...
void run_detector(int argc, char **argv)
{
    ...
    if (argc < 4) {
        fprintf(stdout, "usage: %s %s [train/test/valid/demo/map] [data] [cfg] [weights (optional)]\n", argv[0], argv[1]);
        return;
    }
